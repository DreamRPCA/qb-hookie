...
RegisterNetEvent('qb-vanillaunicorn:Client:stash', function()
    if onDuty and PlayerData.job.name == "hookies" then
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "hookiesstash_"..QBCore.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "hookiesstash_"..QBCore.Functions.GetPlayerData().citizenid)
    end
end)
...
if Config.debug then 
    RegisterCommand('ysm:test', function() -- debug command
        TriggerServerEvent('ysm:test')
    end, false)
end

RegisterNetEvent('qb-hookies:Client:accessBarMenu', function(data) -- Access bar menu, exploit checks
...
RegisterNetEvent('qb-hookies:Client:purchaseDrink', function(data) -- purchase drinks, exploit check
    name = data.type.name
    ...
    if name and price then 
        barConfirmation = {
            ...
            },
            {
                ...
                params = {
                    isServer = true,
                    event = "qb-hookies:Server:handleItem",
                    args = {
                        type = data.type.item,
...
RegisterNetEvent('qb-hookies:Client:barMenu', function() -- Open bar menu
    exports["qb-menu"]:openMenu(drinks)
end)

RegisterNetEvent('qb-hookies:Client:employeeManagement', function() -- Open boss menu
